...
class TTAlertActionSheetInteractiveTransition: TTAlertBaseInteractiveTransition {
    ...
    public override func classForGestureRecognizer() -> TTAlertBaseTransitionGestureRecognizer.Type {
        return TTAlertActionSheetTransitionGestureRecognizer.self
    }
...
//        let containerView = transitionContext.containerView
        ...
        if let alertViewController = viewController as? TTAlertViewController, let alertContainerView = alertViewController.containerView {
            ...
            let currentTopOffset = TTAlertBaseInteractiveTransition.valueBetween(start: startY,
                                                                                 andEnd: endY,
...
extension TTAlertActionSheetInteractiveTransition  {
    ...
    @objc override public func handlePan(_ recognizer: UIPanGestureRecognizer)  {
        ...
        if recognizer.state == .began {
            ...
            if let alertViewController = modalViewController as? TTAlertViewController,
...
//        toViewController?.beginAppearanceTransition(true, animated: true)
        if transitionType == .present {
            ...
            /** SHOW ANIMATION **/
            if let alertViewController = toViewController as? TTAlertViewController, let containerView = containerView   {
                ...
                UIView.animate(withDuration: duration, delay: 0.0, usingSpringWithDamping: 1.0, initialSpringVelocity: 0.0, options: [.curveEaseIn, .beginFromCurrentState], animations: { [weak self] () -> Void in
