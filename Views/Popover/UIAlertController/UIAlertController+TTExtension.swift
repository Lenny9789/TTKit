...
extension UIAlertController {
    ...
    /// 系统Alert弹框
    public class func showAlertWithTitle(
        title: String?,
        message: String?,
        items: [(String, UIAlertAction.Style)],
        dismissClosure: @escaping (Int) -> Void
    ) {
        let alert = UIAlertController(title: title, message: message, preferredStyle: .alert)
        for (index, item) in items.enumerated() {
            let item = UIAlertAction(title: item.0, style: item.1) { (action) in
                dismissClosure(index)
            }
            alert.addAction(item)
        }
        TTRouter.shared.currentViewController()?.showDetailViewController(alert, sender: nil)
    }
}
...
//            UIApplication.shared.keyWindow?.rootViewController?.present(self, animated: animated, completion: completion)
            UIApplication.shared.keyWindow?.rootViewController?.showDetailViewController(self, sender: nil)
            if vibrate {
